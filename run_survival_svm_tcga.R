...
source("single_kernel_survival_train.R")
source("single_kernel_survival_test.R")
...
for (cohort in cohorts) {
  ...
  for (replication in 1:100) {
    if (file.exists(sprintf("%s/%s/svm_pathway_%s_measure_CI_replication_%d_result.RData", result_path, cohort, pathway, replication)) == FALSE) {
      ...
      for (fold in 1:fold_count) {
        ...
        D_train <- pdist(X_train, X_train)
        D_test <- pdist(X_test, X_train)
        sigma <- mean(D_train)
        K_train <- exp(-D_train^2 / (2 * sigma^2))
        K_test <- exp(-D_test^2 / (2 * sigma^2))
        ...
        for (C in C_set) {
          ...
          state <- single_kernel_survival_train(K_train, y_train, delta_train, parameters)
          prediction <- single_kernel_survival_test(K_test, state)
      ...
      save("result", file = sprintf("%s/%s/svm_pathway_%s_measure_CI_replication_%d_result.RData", result_path, cohort, pathway, replication))
    }
  }
}
